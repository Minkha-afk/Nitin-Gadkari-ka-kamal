...
import type { Hazard, TripResponse } from '@/app/api/trip/route';
    ...
    <Panel flush style={{ flexShrink: 0 }}>
      ...
      <PanelBody style={{ padding: 16 }}>
        <form onSubmit={check} style={{ display: 'flex', gap: 10, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <Field label="Pick-up" value={source} onChange={setSource} placeholder="Dispur, Guwahati" dot="#0A0A0A" />
          <button
            type="button"
            onClick={swap}
          ...
          <Field label="Drop" value={dest} onChange={setDest} placeholder="Six Mile, Guwahati" dot={color.mark} />
...
/* ── one input, with a coloured dot like a ride app ─────────────── */
